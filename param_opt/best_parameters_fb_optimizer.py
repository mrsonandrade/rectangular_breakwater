...
class Optimization():
    ...
    def optimize(self):
        ...
        np.savetxt("changing_parameters_history_seed_%d_pop_%d_F_%d_CR_%d.csv" % (self.seed, self.population_size, int(self.F*10), int(self.CR*10)), data, delimiter=",")
        pos = result.X
        self.pos = pos
        final_result = []
...
for seed in [1]:
    for population_size in [10, 20, 30]:
        for F in [0.8, 0.9, 1.0]:
            for CR in [0.2, 0.3, 0.4, 0.7, 0.8, 0.9]:
                ...
                np.savetxt("changing_parameters_seed_%d_pop_%d_F_%d_CR_%d.csv" % (seed, population_size, int(F*10), int(CR*10)), b_list, delimiter=",")
